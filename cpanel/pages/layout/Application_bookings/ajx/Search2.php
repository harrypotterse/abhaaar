...
try {
    $Search = $_POST['Search'];
    $Query = "select  * from visits  where Name LIKE ?"
            . "UNION "
            . " select  * from visits  where Email LIKE ?"
            . " UNION"
            . " select  * from visits  where Contact_Data LIKE ?"
            . " UNION"
            . " select  * from visits  where Membership_No LIKE ?"
            . " UNION "
            . "select  * from visits  where ID_Number LIKE ?"
            . " UNION"
            . " select  * from visits  where Membership_start LIKE ?"
            . " UNION"
            . " select  * from visits  where Membership_expiration LIKE ?"
            . " UNION"
            . " select  * from visits  where Entry_card LIKE ?"
            . " UNION"
            . " select  * from visits  where Type_Membership  LIKE ?"
            . " UNION"
            . " select  * from visits  where  Membership_status LIKE ?";
    ...
    $sth->bindValue(7, '%' . $Search . '%', PDO::PARAM_STR);
    $sth->bindValue(8, '%' . $Search . '%', PDO::PARAM_STR);
    $sth->bindValue(9, '%' . $Search . '%', PDO::PARAM_STR);
    $sth->bindValue(10, '%' . $Search . '%', PDO::PARAM_STR);
    $sth->execute();
    $result = $sth->fetchAll();
if (count($result) > 0) {
